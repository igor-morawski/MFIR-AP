...
def intermediate2processed(dataset_path, destination_parent_path, development_subjects, test_subjects=None, keras_model=None):
    '''
    keras_model : Keras model for feature extraction, if None ['array_IDRGB'] will be None], must be initialized
    '''
    ### BATCH MAKER START ###
    mu, sigma = _compute_devel_mu_sigma(
        dataset_path, destination_parent_path, development_subjects)
    # save to dict
    ...
    _intermediate2processed_development(
        dataset_path, destination_parent_path, development_subjects, mu, sigma, keras_model)
    vb.print_general(
        "[WARNING] intermediate2processed produces only development set (train+valid), test set is depreciated because testing is now done on full clips, not snippets.")
    ### BATCH MAKER END ###


def _intermediate2processed_development(dataset_path, destination_parent_path, subjects, mu, sigma, rgb_model):
    _batch_maker(dataset_path, destination_parent_path,
                 subjects, "development", mu, sigma, rgb_model)
...
def _intermediate2processed_test(dataset_path, destination_parent_path, subjects, mu, sigma, rgb_model):
    ...
    '''
    _batch_maker(dataset_path, destination_parent_path,
                 subjects, "test", mu, sigma, rgb_model)
...
def _batch_maker(dataset_path, destination_parent_path, subjects, devel_or_test, TPA_mean, TPA_std, rgb_model):
    ...
    pos_fp_list, neg_fp_list = get_pos_neg_fp_lists(dataset_path)
    _, name = os.path.split(dataset_path)
    batch_maker_dest = os.path.join(
        destination_parent_path, name, devel_or_test)
    ...
    if rgb_model:
        rgb_shape = MFIRAP.d00_utils.io.read_json_key(
            os.path.join(dataset_path, 'samples.nfo'), 'rgb_shape')
    ...
    # 4. Main loop
    print_general('Processing and saving {} samples...'.format(
        len(pos_fp_list + neg_fp_list)))
    for fp in pos_fp_list + neg_fp_list:
        ...
        tpa1, tpa2, tpa3 = [standarize_TPA(t, TPA_mean, TPA_std) for t in [
            tpa1, tpa2, tpa3]]
        sample_keys = list(sample.keys())
        dict2save = {'array_ID121': tpa1, 'array_ID122': tpa2, 'array_ID123': tpa3,
                     'array_IDRGB': rgb, 'mu': TPA_mean, "sigma": TPA_std}
        for key in sample_keys:
            if key not in dict2save.keys():
                dict2save[key] = sample[key]
        output = os.path.join(batch_maker_dest, str(
            int(np.argmax(sample['one_hot']))), tail)
        ensure_parent_exists(output)
        print_specific("Writing {}".format(output))
        np.savez_compressed(output, **dict2save)
    print_general('Processed and saved {} samples in {} set.'.format(
        len(pos_fp_list + neg_fp_list), devel_or_test))
    ### BATCH MAKER END ###
